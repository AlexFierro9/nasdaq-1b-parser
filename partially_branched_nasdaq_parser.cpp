...
alignas(64) static const uint8_t ITCH_LENGTHS[256] = {
    ['S'] = 12, ['R'] = 38, ['H'] = 25, ['Y'] = 20, ['L'] = 26, ['V'] = 35, ['W'] = 12, ['K'] = 28, ['J'] = 35, ['h'] = 21, ['A'] = 36, ['F'] = 40, ['E'] = 31, ['C'] = 36, ['X'] = 23, ['D'] = 19, ['U'] = 35, ['P'] = 44, ['Q'] = 40, ['B'] = 19, ['I'] = 50, ['N'] = 27, ['O'] = 48};
...
struct ITCHMessage<'R'>
{
    static __attribute__((noinline)) void process(const uint8_t *buf)
    {
        ...
        uint64_t time_stamp = load_be48(buf + 5);
        uint64_t raw_stock;
        std::memcpy(&raw_stock, buf + 11, 8);
        uint32_t round_lot_size = load_be32(buf + 21);
        uint16_t raw_issue_sub;
        std::memcpy(&raw_issue_sub, buf + 27, 2);
        ...
        tl_sink += stock_locate ^ tracking_number ^ time_stamp ^ raw_stock ^ round_lot_size ^ raw_issue_sub ^ etp_leverage ^ market_cat ^ fin_status ^ rounds_only ^ issue_cls ^ authenticity ^ short_sale ^ ipo_flag ^ luld_tier ^ etp_flag ^ inverse;
    }
};
...
struct ITCHMessage<'H'>
{
    static __attribute__((noinline)) void process(const uint8_t *buf)
    {
        ...
        std::memcpy(&raw_stock, buf + 11, 8);
        uint32_t raw_reason;
        std::memcpy(&raw_reason, buf + 21, 4);
        uint8_t trading_state = buf[19];
        uint8_t reserved = buf[20];

        tl_sink += stock_locate ^ tracking_number ^ time_stamp ^ raw_stock ^ raw_reason ^ trading_state ^ reserved;
    }
};
...
struct ITCHMessage<'Y'>
{
    static __attribute__((noinline)) void process(const uint8_t *buf)
    {
        ...
        tl_sink += stock_locate ^ tracking_number ^ time_stamp ^ raw_stock ^ reg_sho_action;
    }
};
...
struct ITCHMessage<'L'>
{
    static __attribute__((noinline)) void process(const uint8_t *buf)
    {
        ...
        uint64_t time_stamp = load_be48(buf + 5);
        uint32_t raw_mpid;
        std::memcpy(&raw_mpid, buf + 11, 4);
        uint64_t raw_stock;
        std::memcpy(&raw_stock, buf + 15, 8);
        uint8_t primary_mkt = buf[23];
        uint8_t mm_code = buf[24];
        uint8_t mm_state = buf[25];

        tl_sink += stock_locate ^ tracking_number ^ time_stamp ^ raw_mpid ^ raw_stock ^ primary_mkt ^ mm_code ^ mm_state;
    }
};
...
struct ITCHMessage<'V'>
{
    static __attribute__((noinline)) void process(const uint8_t *buf)
    {
        ...
        tl_sink += stock_locate ^ tracking_number ^ time_stamp ^ level1 ^ level2 ^ level3;
    }
};

// 'W' — MWCB Status Message (12 bytes)
...
struct ITCHMessage<'K'>
{
    static __attribute__((noinline)) void process(const uint8_t *buf)
    {
        ...
        tl_sink += stock_locate ^ tracking_number ^ time_stamp ^ raw_stock ^ ipo_quotation ^ ipo_price ^ ipo_qualifier;
    }
};
...
struct ITCHMessage<'J'>
{
    static __attribute__((noinline)) void process(const uint8_t *buf)
    {
        ...
        tl_sink += stock_locate ^ tracking_number ^ time_stamp ^ raw_stock ^ collar_ref_price ^ upper_collar ^ lower_collar ^ collar_extension;
    }
};

// 'h' — Operational Halt Message (21 bytes)
...
struct ITCHMessage<'h'>
{
    static __attribute__((noinline)) void process(const uint8_t *buf)
    {
        ...
        tl_sink += stock_locate ^ tracking_number ^ time_stamp ^ raw_stock ^ market_code ^ halt_code;
    }
};
...
struct ITCHMessage<'A'>
{
    static __attribute__((always_inline)) void process(const uint8_t *buf)
    {
        ...
        uint64_t raw_stock;
        std::memcpy(&raw_stock, buf + 24, 8);
        uint32_t price = load_be32(buf + 32);
        uint8_t buy_sell = buf[19];

        tl_sink += stock_locate ^ tracking_number ^ time_stamp ^ order_ref ^ shares ^ raw_stock ^ price ^ buy_sell;
    }
};
...
struct ITCHMessage<'F'>
{
    static __attribute__((always_inline)) void process(const uint8_t *buf)
    {
        ...
        std::memcpy(&raw_stock, buf + 24, 8);
        uint32_t price = load_be32(buf + 32);
        uint32_t raw_attribution;
        std::memcpy(&raw_attribution, buf + 36, 4);
        uint8_t buy_sell = buf[19];

        tl_sink += stock_locate ^ tracking_number ^ time_stamp ^ order_ref ^ shares ^ raw_stock ^ price ^ raw_attribution ^ buy_sell;
    }
};
...
struct ITCHMessage<'E'>
{
    static __attribute__((always_inline)) void process(const uint8_t *buf)
    {
        ...
        tl_sink += stock_locate ^ tracking_number ^ time_stamp ^ order_ref ^ executed_shares ^ match_number;
    }
};
...
struct ITCHMessage<'C'>
{
    static __attribute__((always_inline)) void process(const uint8_t *buf)
    {
        ...
        tl_sink += stock_locate ^ tracking_number ^ time_stamp ^ order_ref ^ executed_shares ^ match_number ^ execution_price ^ printable;
    }
};
...
struct ITCHMessage<'X'>
{
    static __attribute__((always_inline)) void process(const uint8_t *buf)
    {
        ...
        tl_sink += stock_locate ^ tracking_number ^ time_stamp ^ order_ref ^ cancelled_shares;
    }
};
...
struct ITCHMessage<'U'>
{
    static __attribute__((always_inline)) void process(const uint8_t *buf)
    {
        ...
        tl_sink += stock_locate ^ tracking_number ^ time_stamp ^ order_ref ^ new_order_ref ^ shares ^ price;
    }
};
...
struct ITCHMessage<'P'>
{
    static __attribute__((always_inline)) void process(const uint8_t *buf)
    {
        ...
        tl_sink += stock_locate ^ tracking_number ^ time_stamp ^ order_ref ^ shares ^ raw_stock ^ price ^ match_number ^ buy_sell;
    }
};
...
struct ITCHMessage<'Q'>
{
    static inline void process(const uint8_t *buf)
    {
        ...
        uint64_t raw_stock;
        std::memcpy(&raw_stock, buf + 19, 8);
        uint32_t cross_price = load_be32(buf + 27);
        uint64_t match_number = load_be64(buf + 31);
        uint8_t cross_type = buf[39];

        tl_sink += stock_locate ^ tracking_number ^ time_stamp ^ shares ^ raw_stock ^ cross_price ^ match_number ^ cross_type;
    }
};
...
struct ITCHMessage<'I'>
{
    static inline void process(const uint8_t *buf)
    {
        ...
        tl_sink += stock_locate ^ tracking_number ^ time_stamp ^ paired_shares ^ imbalance_shares ^ raw_stock ^ far_price ^ near_price ^ current_ref ^ imbalance_dir ^ cross_type ^ price_var;
    }
};
...
struct ITCHMessage<'N'>
{
    static void process(const uint8_t *buf)
    {
        ...
        tl_sink += stock_locate ^ tracking_number ^ time_stamp ^ raw_stock ^ interest_flag;
    }
};
...
struct ITCHMessage<'O'>
{
    static inline void process(const uint8_t *buf)
    {
        ...
        tl_sink += stock_locate ^ tracking_number ^ time_stamp ^ raw_stock ^ min_price ^ max_price ^ near_exec_price ^ near_exec_time ^ lower_collar ^ upper_collar ^ open_elig;
    }
};
                                    ...
                                    uint64_t msg_count)
{
    ...
    auto to_gbps = [&](double s)
    { return (file_size / 1073741824.0) / s; };
    auto to_mmsg = [&](double s)
    { return (msg_count / 1e6) / s; };
    auto to_lat_ns = [&](double s)
    { return (s * 1e9) / msg_count; };
...
RunResult run_benchmark(uint8_t *data, size_t total_file_size, bool silent)
{
    ...
    while (offset < total_file_size)
    {
        ...
        if (type == 'A')
            ITCHMessage<'A'>::process(data + offset);
        else if (type == 'P')
            ITCHMessage<'P'>::process(data + offset);
        else if (type == 'E')
            ITCHMessage<'E'>::process(data + offset);
        else
    ...
    flush_sink(); // fold thread-local sink into global
...
#pragma omp parallel reduction(+ : total_msgs)
    {
        ...
        if (tid > 0)
            my_start = sync_to_next_message(data, my_start, my_end);
        ...
        flush_sink(); // each thread flushes its own tl_sink
    }
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> diff = end - start;
...
int main()
{
    ...
    for (int i = 0; i < ITERATIONS; ++i)
    {
        ...
        if (i == 0)
            final_msg_count = res_s.msg_count;
